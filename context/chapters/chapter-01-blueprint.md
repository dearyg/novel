...
**Actual footer in v5 prose (source of truth):** a C++ Discovery Log block —
...
This block is **retained** in the finalized prose. Ch 1 is one of the eight C++ blocks preserved under revision-plan P1.7; it is NOT a stale directive. The broader "Mandatory Chapter Footer (Code Block)" instruction was superseded by P1.7's footer-diversification pass — blueprints that still say "mandatory C++ footer" are stale, but for Ch 1 the prose and blueprint happen to agree. Do not treat this footer as a restoration target; trust the prose file as the canonical artifact. Future chapter blueprints that mandate C++ footers but whose prose has replaced them with an in-world artifact (Concierge log, Maya's notebook, child's drawing, etc.) should likewise defer to the prose under P1.7.
